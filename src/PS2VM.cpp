...
CPS2VM::CPS2VM()
    : m_nStatus(PAUSED), m_nEnd(false), m_pad(NULL), m_singleStepEe(false), m_singleStepIop(false),
      m_singleStepVu0(false), m_singleStepVu1(false), m_vblankTicks(0), m_inVblank(false), m_eeExecutionTicks(0),
      m_iopExecutionTicks(0), m_spuUpdateTicks(SPU_UPDATE_TICKS),
...
std::future<bool> CPS2VM::SaveState(const fs::path &statePath)
{
    ...
    m_mailBox.SendCall([this, promise, statePath]() {
...
void CPS2VM::CreateVM()
{
    ...
    m_OnRequestLoadExecutableConnection = m_ee->m_os->OnRequestLoadExecutable.Connect(
        std::bind(&CPS2VM::ReloadExecutable, this, std::placeholders::_1, std::placeholders::_2));
...
void CPS2VM::ResetVM()
{
...

    {
        ...
        iopOs->GetIoman()->RegisterDevice(
            "rom0", std::make_shared<Iop::Ioman::CPreferenceDirectoryDevice>(PREF_PS2_ROM0_DIRECTORY));
        iopOs->GetIoman()->RegisterDevice(
            "host", std::make_shared<Iop::Ioman::CPreferenceDirectoryDevice>(PREF_PS2_HOST_DIRECTORY));
        iopOs->GetIoman()->RegisterDevice(
            "host0", std::make_shared<Iop::Ioman::CPreferenceDirectoryDevice>(PREF_PS2_HOST_DIRECTORY));
        iopOs->GetIoman()->RegisterDevice(
            "mc0", std::make_shared<Iop::Ioman::CPreferenceDirectoryDevice>(PREF_PS2_MC0_DIRECTORY));
        iopOs->GetIoman()->RegisterDevice(
            "mc1", std::make_shared<Iop::Ioman::CPreferenceDirectoryDevice>(PREF_PS2_MC1_DIRECTORY));
        iopOs->GetIoman()->RegisterDevice("cdrom",
                                          Iop::CIoman::DevicePtr(new Iop::Ioman::COpticalMediaDevice(m_cdrom0)));
        iopOs->GetIoman()->RegisterDevice("cdrom0",
                                          Iop::CIoman::DevicePtr(new Iop::Ioman::COpticalMediaDevice(m_cdrom0)));
        iopOs->GetIoman()->RegisterDevice("hdd0", std::make_shared<Iop::Ioman::CHardDiskDevice>());

        iopOs->GetLoadcore()->SetLoadExecutableHandler(
            std::bind(&CPS2OS::LoadExecutable, m_ee->m_os, std::placeholders::_1, std::placeholders::_2));
    }
...
void CPS2VM::CreateGsHandlerImpl(const CGSHandler::FactoryFunction &factoryFunction)
{
    ...
    m_ee->m_gs->SendGSCall(
        [this]() { static_cast<CEeExecutor *>(m_ee->m_EE.m_executor.get())->AttachExceptionHandlerToThread(); });
    if (gs) {
        m_ee->m_gs->Copy(gs);
        gs->Release();
        delete gs;
...
#ifdef DEBUGGER_INCLUDED
        if (m_singleStepEe || m_singleStepVu0 || m_singleStepVu1)
            break;
        if (m_ee->m_EE.m_executor->MustBreak())
            break;
#endif
    }
}
...
void CPS2VM::UpdateIop()
{
    ...
    while (m_iopExecutionTicks > 0) {
        ...
        if (m_singleStepIop)
            break;
        if (m_iop->m_cpu.m_executor->MustBreak())
            break;
#endif
    }
}
...
#ifdef DEBUGGER_INCLUDED
            if (m_ee->m_EE.m_executor->MustBreak() || m_iop->m_cpu.m_executor->MustBreak() ||
                m_ee->m_VU1.m_executor->MustBreak() || m_singleStepEe || m_singleStepIop || m_singleStepVu0 ||
                m_singleStepVu1) {
                m_nStatus       = PAUSED;

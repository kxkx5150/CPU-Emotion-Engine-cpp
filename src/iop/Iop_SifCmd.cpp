...
CSifCmd::CSifCmd(CIopBios &bios, CSifMan &sifMan, CSysmem &sysMem, uint8 *ram)
    : m_bios(bios), m_sifMan(sifMan), m_sysMem(sysMem), m_ram(ram)
{
...
void CSifCmd::Invoke(CMIPS &context, unsigned int functionId)
{
    switch (functionId) {
        case 6:
            context.m_State.nGPR[CMIPS::V0].nV0 = SifGetSreg(context.m_State.nGPR[CMIPS::A0].nV0);
            break;
        case 7:
            SifSetSreg(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0);
            break;
        case 8:
            context.m_State.nGPR[CMIPS::V0].nV0 =
                SifSetCmdBuffer(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0);
            break;
        case 10:
            SifAddCmdHandler(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0,
                             context.m_State.nGPR[CMIPS::A2].nV0);
            break;
        case 12:
        case 13:
            context.m_State.nGPR[CMIPS::V0].nV0 =
                SifSendCmd(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0,
        ...
        case 17:
            ...
            break;
        case 18:
            context.m_State.nGPR[CMIPS::V0].nV0 = SifCheckStatRpc(context.m_State.nGPR[CMIPS::A0].nV0);
            break;
        case 19:
            SifSetRpcQueue(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0);
            break;
        case 20:
            context.m_State.nGPR[CMIPS::V0].nD0 =
                static_cast<int32>(SifGetNextRequest(context.m_State.nGPR[CMIPS::A0].nV0));
            break;
        case 21:
        ...
        case 23:
            context.m_State.nGPR[CMIPS::V0].nV0 =
                SifGetOtherData(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0,
                                context.m_State.nGPR[CMIPS::A2].nV0, context.m_State.nGPR[CMIPS::A3].nV0,
                                context.m_pMemoryMap->GetWord(context.m_State.nGPR[CMIPS::SP].nV0 + 0x10));
            break;
        case 24:
            context.m_State.nGPR[CMIPS::V0].nV0 =
                SifRemoveRpc(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0);
            break;
        case 25:
            context.m_State.nGPR[CMIPS::V0].nV0 = SifRemoveRpcQueue(context.m_State.nGPR[CMIPS::A0].nV0);
            break;
        case 28:
        case 29:
            context.m_State.nGPR[CMIPS::V0].nV0 =
                SifSendCmdIntr(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0,
                               context.m_State.nGPR[CMIPS::A2].nV0, context.m_State.nGPR[CMIPS::A3].nV0,
                               context.m_pMemoryMap->GetWord(context.m_State.nGPR[CMIPS::SP].nV0 + 0x10),
                               context.m_pMemoryMap->GetWord(context.m_State.nGPR[CMIPS::SP].nV0 + 0x14),
            ...
            break;
        case CUSTOM_FINISHEXECREQUEST:
            FinishExecRequest(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0);
            break;
        case CUSTOM_FINISHBINDRPC:
            FinishBindRpc(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0);
            break;
        case CUSTOM_FINISHEXECCMD:
            FinishExecCmd();
            break;
        case CUSTOM_SLEEPTHREAD:
            ...
            break;
        case CUSTOM_DELAYTHREAD:
            DelayThread(context.m_State.nGPR[CMIPS::A0].nV0);
            break;
        default:
...
void CSifCmd::FinishExecCmd()
{
    ...
    memmove(pendingCmdBuffer, pendingCmdBuffer + commandPacketSize,
            PENDING_CMD_BUFFER_SIZE - moduleData->pendingCmdBufferSize);
...
void CSifCmd::ProcessNextDynamicCommand()
{
    ...
    if ((cmdBufferAddr != 0) && (cmd < cmdBufferLen)) {
        ...
        CLog::GetInstance().Print(LOG_NAME,
                                  "Calling SIF command handler for command 0x%08X at 0x%08X with data 0x%08X.\r\n",
        ...
        if (cmdDataEntry.sifCmdHandler != 0) {
            ...
            // That's not the case here though, so we try for the same effect by calling the handler outside of an
            // interrupt.
...
void CSifCmd::SifAddCmdHandler(uint32 pos, uint32 handler, uint32 data)
{
    CLog::GetInstance().Print(
        LOG_NAME, FUNCTION_SIFADDCMDHANDLER "(pos = 0x%08X, handler = 0x%08X, data = 0x%08X);\r\n", pos, handler, data);
...
uint32 CSifCmd::SifSendCmd(uint32 commandId, uint32 packetPtr, uint32 packetSize, uint32 srcExtraPtr,
                           uint32 dstExtraPtr, uint32 sizeExtra)
{
    CLog::GetInstance().Print(LOG_NAME,
                              FUNCTION_SIFSENDCMD
                              "(commandId = 0x%08X, packetPtr = 0x%08X, packetSize = 0x%08X, srcExtraPtr = 0x%08X, "
                              "dstExtraPtr = 0x%08X, sizeExtra = 0x%08X);\r\n",
...
void CSifCmd::SifBindRpc(CMIPS &context)
{
    ...
    CLog::GetInstance().Print(LOG_NAME,
                              FUNCTION_SIFBINDRPC "(clientDataAddr = 0x%08X, serverId = 0x%08X, mode = 0x%08X);\r\n",
...
void CSifCmd::SifRegisterRpc(CMIPS &context)
{
    ...
    CLog::GetInstance().Print(LOG_NAME,
                              FUNCTION_SIFREGISTERRPC
                              "(serverData = 0x%08X, serverId = 0x%08X, function = 0x%08X, buffer = 0x%08X, cfunction "
                              "= 0x%08X, cbuffer = 0x%08X, queue = 0x%08X);\r\n",
...
void CSifCmd::SifExecRequest(CMIPS &context)
{
    ...
    CLog::GetInstance().Print(
        LOG_NAME,
        FUNCTION_SIFEXECREQUEST
        "(serverData = 0x%08X, serverId=0x%x, function=0x%x, rid=0x%x, buffer=0x%x, rsize=0x%x);\r\n",
        serverDataAddr, serverData->serverId, serverData->function, serverData->rid, serverData->buffer,
        serverData->rsize);
...
{
    CLog::GetInstance().Print(LOG_NAME,
                              FUNCTION_SIFGETOTHERDATA
                              "(packetPtr = 0x%08X, src = 0x%08X, dst = 0x%08X, size = 0x%08X, mode = %d);\r\n",
                              packetPtr, src, dst, size, mode);
    m_sifMan.GetOtherData(dst, src, size);
    return 0;
}

uint32 CSifCmd::SifSendCmdIntr(uint32 commandId, uint32 packetPtr, uint32 packetSize, uint32 srcExtraPtr,
                               uint32 dstExtraPtr, uint32 sizeExtra, uint32 callbackPtr, uint32 callbackDataPtr)
{
    CLog::GetInstance().Print(
        LOG_NAME,
        FUNCTION_SIFSENDCMDINTR
        "(commandId = 0x%08X, packetPtr = 0x%08X, packetSize = 0x%08X, srcExtraPtr = 0x%08X, dstExtraPtr = 0x%08X, "
        "sizeExtra = 0x%08X, callbackPtr = 0x%08X, callbackDataPtr = 0x%08X);\r\n",

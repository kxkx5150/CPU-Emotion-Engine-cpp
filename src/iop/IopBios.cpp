...
CIopBios::CIopBios(CMIPS &cpu, uint8 *ram, uint32 ramSize, uint8 *spr)
    : m_cpu(cpu), m_ram(ram), m_ramSize(ramSize), m_spr(spr), m_threadFinishAddress(0), m_returnFromExceptionAddress(0),
      m_idleFunctionAddress(0), m_moduleStarterProcAddress(0), m_alarmThreadProcAddress(0), m_vblankHandlerAddress(0),
      ...
      m_currentThreadId(reinterpret_cast<uint32 *>(m_ram + BIOS_CURRENT_THREAD_ID_BASE))
{
    ...
    static_assert(BIOS_SYSTEM_INTRHANDLER_TABLE_BASE > CIopBios::CONTROL_BLOCK_START,
                  "Intr handler table is outside reserved block");
}
...
void CIopBios::Reset(const Iop::SifManPtr &sifMan)
{
    ...
    {
        m_sysmem = std::make_shared<Iop::CSysmem>(m_ram, CONTROL_BLOCK_END, m_ramSize, m_memoryBlocks, *m_stdio,
                                                  *m_ioman, *m_sifMan);
...
void CIopBios::SaveState(Framework::CZipArchiveWriter &archive)
{
    ...
    archive.InsertFile(
        new CMemoryStateFile(STATE_MODULESTARTREQUESTS, m_moduleStartRequests, sizeof(m_moduleStartRequests)));
}

void CIopBios::LoadState(Framework::CZipArchiveReader &archive)
{
    // Remove all dynamic modules
    for (auto modulePairIterator = m_modules.begin(); modulePairIterator != m_modules.end();) {
    ...
    {
        for (auto structIterator(modulesFile.GetStructBegin()); structIterator != modulesFile.GetStructEnd();
             structIterator++) {
    ...
    for (const auto &loadedModule : m_loadedModules) {
        ...
        if (loadedModule->state == MODULE_STATE::HLE) {
            auto moduleIterator = std::find_if(m_hleModules.begin(), m_hleModules.end(), [&](const auto &modulePair) {
...
void CIopBios::RequestModuleStart(MODULESTARTREQUEST_SOURCE requestSource, bool stopRequest, uint32 moduleId,
                                  const char *path, const char *args, unsigned int argsLength)
{
...
{
    static const auto pushToStack = [](uint8 *dst, uint32 &stackAddress, const uint8 *src, uint32 size) {
    ...
    if (!moduleStartRequest->stopRequest) {
        ...
        paramList.push_back(pushToStack(m_ram, m_cpu.m_State.nGPR[CMIPS::SP].nV0, reinterpret_cast<const uint8 *>(path),
                                        static_cast<uint32>(strlen(path)) + 1));
        if (argsLength != 0) {
            uint32       stackArgsBase = pushToStack(m_ram, m_cpu.m_State.nGPR[CMIPS::SP].nV0,
                                                     reinterpret_cast<const uint8 *>(args), argsLength);
        ...
        for (auto param = paramList.rbegin(); paramList.rend() != param; param++) {
            m_cpu.m_State.nGPR[CMIPS::A1].nV0 =
                pushToStack(m_ram, m_cpu.m_State.nGPR[CMIPS::SP].nV0, reinterpret_cast<const uint8 *>(&(*param)), 4);
        }
    } else {
...
int32 CIopBios::StartModule(MODULESTARTREQUEST_SOURCE requestSource, uint32 loadedModuleId, const char *path,
                            const char *args, uint32 argsLength)
{
    auto loadedModule = m_loadedModules[loadedModuleId];
    if (loadedModule == nullptr) {
        return -1;
...
void CIopBios::ProcessModuleReset(const std::string &imagePath)
{
    ...
    if (!found)
        found = TryGetImageVersionFromPath(imagePath, &imageVersion);
    assert(found);
...
bool CIopBios::TryGetImageVersionFromContents(const std::string &imagePath, unsigned int *result)
{
    ...
    auto tryMatchVersionPattern = [result](auto moduleVersionString, const std::string &patternString) {
                              ...
                              uint32 attributes)
{
    ...
    CLog::GetInstance().Print(
        LOGNAME, "%d: CreateThread(threadProc = 0x%08X, priority = %d, stackSize = 0x%08X, attributes = 0x%08X);\r\n",
...
uint32 CIopBios::CancelAlarm(uint32 alarmFunction, uint32 param)
{
    ...
    for (auto thread : m_threads) {
        ...
        if (thread->status == THREAD_STATUS_DORMANT)
            continue;
        if (thread->optionData != alarmFunction)
            continue;
        if (thread->threadProc == m_alarmThreadProcAddress) {
            alarmThreadId = thread->id;
            break;
...
{
#ifdef _DEBUG
    CLog::GetInstance().Print(LOGNAME,
                              "%d: ReferThreadStatus(threadId = %d, statusPtr = 0x%08X, inInterrupt = %d);\r\n",
...
int32 CIopBios::ReleaseWaitThread(uint32 threadId, bool inInterrupt)
{
    ...
    if ((thread->status == THREAD_STATUS_RUNNING) || (thread->status == THREAD_STATUS_DORMANT)) {
...
int32 CIopBios::FindVblankHandlerByLineAndPtr(uint32 startEnd, uint32 handlerPtr)
{
    ...
    for (unsigned int i = 0; i < MAX_VBLANKHANDLER; i++) {
        if (m_vblankHandlers[i] != nullptr && m_vblankHandlers[i]->handler == handlerPtr &&
            m_vblankHandlers[i]->type == startEnd) {
            handlerId = i;
            break;
        }
    }
...
#ifdef _DEBUG
    CLog::GetInstance().Print(
        LOGNAME, "%i: CreateSemaphore(initialCount = %i, maxCount = %i, optionData = 0x%08X, attributes = 0x%08X);\r\n",
        ...
        if (thread->waitEventFlag == eventId) {
            bool success = ProcessEventFlag(thread->waitEventFlagMode, eventFlag->value, thread->waitEventFlagMask,
                                            (thread->waitEventFlagResultPtr != 0)
                                                ? reinterpret_cast<uint32 *>(m_ram + thread->waitEventFlagResultPtr)
                                                : nullptr);
            if (success) {
...
#ifdef _DEBUG
    CLog::GetInstance().Print(LOGNAME,
                              "%d: WaitEventFlag(eventId = %d, value = 0x%08X, mode = 0x%02X, resultPtr = 0x%08X);\r\n",
...
#ifdef _DEBUG
    CLog::GetInstance().Print(LOGNAME,
                              "%d: PollEventFlag(eventId = %d, value = 0x%08X, mode = 0x%02X, resultPtr = 0x%08X);\r\n",
                              m_currentThreadId.Get(), eventId, value, mode, resultPtr);
#endif
...
uint32 CIopBios::AllocateFpl(uint32 fplId)
{
    ...
    if (result == KERNEL_RESULT_ERROR_NO_MEMORY) {
        CLog::GetInstance().Warn(LOGNAME,
                                 "No memory left while calling AllocateFpl, should be waiting. (not implemented)");
        assert(false);
    }
    return result;
}
...
void CIopBios::ReleaseWaitCdSync()
{
    for (auto thread : m_threads) {
        if (!thread)
            continue;
        if (thread->status != THREAD_STATUS_WAIT_CDSYNC)
            continue;
...
{
    for (auto handlerIterator = std::begin(m_intrHandlers); handlerIterator != std::end(m_intrHandlers);
         handlerIterator++) {
...
void CIopBios::HandleInterrupt()
{
    if (m_cpu.GenerateInterrupt(m_cpu.m_State.nPC)) {
        ...
        UNION64_32 status(m_cpu.m_pMemoryMap->GetWord(Iop::CIntc::STATUS0),
                          m_cpu.m_pMemoryMap->GetWord(Iop::CIntc::STATUS1));
        UNION64_32 mask(m_cpu.m_pMemoryMap->GetWord(Iop::CIntc::MASK0), m_cpu.m_pMemoryMap->GetWord(Iop::CIntc::MASK1));
        status.f &= mask.f;
        assert(status.f != 0);
        if (status.f == 0) {
            ReturnFromException();
        ...
        // Check if there's an handler to call
        {
            ...
            if (handlerId == -1) {
                ...
                m_cpu.m_State.nGPR[CMIPS::SP].nD0 =
                    (BIOS_INTSTACK_BASE + BIOS_INTSTACK_SIZE) - STACK_FRAME_RESERVE_SIZE;
...
uint32 CIopBios::LoadExecutable(CELF32 &elf, ExecutableRange &executableRange, uint32 baseAddress)
{
    ...
    memcpy(m_ram + baseAddress, elf.GetContent() + programHeader->nOffset, programHeader->nFileSize);
...
{
    auto intrHandlerTable =
        reinterpret_cast<CIopBios::SYSTEM_INTRHANDLER *>(&m_ram[BIOS_SYSTEM_INTRHANDLER_TABLE_BASE]);
...
void CIopBios::SaveDebugTags(Framework::Xml::CNode *root)
{
    ...
    for (const auto &module : m_moduleTags) {
        ...
        moduleNode->InsertAttribute(TAGS_SECTION_IOP_MODULES_MODULE_BEGINADDRESS,
                                    lexical_cast_hex<std::string>(module.begin, 8).c_str());
        moduleNode->InsertAttribute(TAGS_SECTION_IOP_MODULES_MODULE_ENDADDRESS,
                                    lexical_cast_hex<std::string>(module.end, 8).c_str());
...
void CIopBios::PrepareModuleDebugInfo(CELF32 &elf, const ExecutableRange &moduleRange, const std::string &moduleName,
            ...
            if (pStrTab != NULL) {
                ...
                for (unsigned int i = 0; i < nCount; i++) {
                    ...
                    if (symbolSection == NULL)
                        continue;
                    m_cpu.m_Functions.InsertTag(moduleRange.first + symbolSection->nStart + pSym[i].nValue,
                                                (char *)pStrTab + pSym[i].nName);
                    functionAdded = true;
                }
            }
        }
...
BiosDebugModuleInfoIterator CIopBios::FindModuleDebugInfo(const std::string &name)
{
    return std::find_if(std::begin(m_moduleTags), std::end(m_moduleTags),
                        [=](const BIOS_DEBUG_MODULE_INFO &module) { return name == module.name; });
}

BiosDebugModuleInfoIterator CIopBios::FindModuleDebugInfo(uint32 beginAddress, uint32 endAddress)
{
    return std::find_if(std::begin(m_moduleTags), std::end(m_moduleTags), [=](const BIOS_DEBUG_MODULE_INFO &module) {

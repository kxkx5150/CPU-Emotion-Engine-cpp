...
CSifMan::CSifMan() : m_sifSetDmaCallbackHandlerPtr(0)
{
}
    ...
    switch (functionId) {
        case 7:
            context.m_State.nGPR[CMIPS::V0].nD0 =
                static_cast<int32>(SifSetDma(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0));
            break;
        case 8:
        ...
        case 32:
            context.m_State.nGPR[CMIPS::V0].nD0 = static_cast<int32>(
                SifSetDmaCallback(context, context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0,
...
uint32 CSifMan::SifSetDmaCallback(CMIPS &context, uint32 structAddr, uint32 count, uint32 callbackPtr,
                                  uint32 callbackParam)
{
    CLog::GetInstance().Print(LOG_NAME,
                              FUNCTION_SIFSETDMACALLBACK
                              "(structAddr = 0x%08X, count = %d, callbackPtr = 0x%08X, callbackParam = 0x%08X);\r\n",

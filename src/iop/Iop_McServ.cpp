...
#define MC_FILE_ATTR_FOLDER                                                                                            \
    (MC_FILE_0400 | MC_FILE_ATTR_EXISTS | MC_FILE_ATTR_SUBDIR | MC_FILE_ATTR_READABLE | MC_FILE_ATTR_WRITEABLE |       \
     MC_FILE_ATTR_EXECUTABLE)
...
CMcServ::CMcServ(CIopBios &bios, CSifMan &sifMan, CSifCmd &sifCmd, CSysmem &sysMem, uint8 *ram)
    : m_bios(bios), m_sifMan(sifMan), m_sifCmd(sifCmd), m_sysMem(sysMem), m_ram(ram)
{
...
std::string CMcServ::MakeAbsolutePath(const std::string &inputPath)
{
    ...
    for (auto fragIterator = std::rbegin(frags); fragIterator != std::rend(frags); fragIterator++) {
        if (*fragIterator == "..") {
            fragIterator++;
            fragIterator++;
        } else {
...
bool CMcServ::Invoke(uint32 method, uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    switch (method) {
        ...
        case 0x78:
            ...
            //- Baroque
            //- Naruto Shippuden: Ultimate Ninja 5 (if GetInfo doesn't return quickly enough, MC thread is killed and
            //game will hang)
...
void CMcServ::GetInfo(uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    CLog::GetInstance().Print(
        LOG_NAME,
        "GetInfo(port = %i, slot = %i, wantType = %i, wantFreeSpace = %i, wantFormatted = %i, retBuffer = 0x%08X);\r\n",
...
void CMcServ::Read(uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    CLog::GetInstance().Print(LOG_NAME,
                              "Read(handle = %i, size = 0x%08X, bufferAddress = 0x%08X, paramAddress = 0x%08X);\r\n",
...
void CMcServ::Write(uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    CLog::GetInstance().Print(LOG_NAME,
                              "Write(handle = %i, nSize = 0x%08X, bufferAddress = 0x%08X, origin = 0x%08X);\r\n",
...
void CMcServ::GetDir(uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    CLog::GetInstance().Print(
        LOG_NAME, "GetDir(port = %i, slot = %i, flags = %i, maxEntries = %i, tableAddress = 0x%08X, name = '%s');\r\n",
...
bool CMcServ::ReadFast(uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    auto cmd = reinterpret_cast<FILECMD *>(args);
    CLog::GetInstance().Print(
        LOG_NAME, "ReadFast(handle = %d, size = 0x%08X, bufferAddress = 0x%08X, paramAddress = 0x%08X);\r\n",
        cmd->handle, cmd->size, cmd->bufferAddress, cmd->paramAddress);
...
void CMcServ::WriteFast(uint32 *args, uint32 argsSize, uint32 *ret, uint32 retSize, uint8 *ram)
{
    ...
    CLog::GetInstance().Print(
        LOG_NAME, "WriteFast(handle = %d, size = 0x%08X, bufferAddress = 0x%08X, paramAddress = 0x%08X);\r\n",
        cmd->handle, cmd->size, cmd->bufferAddress, cmd->paramAddress);
...
void CMcServ::ProceedReadFast(CMIPS &context)
{
    ...
    context.m_pMemoryMap->SetWord(context.m_State.nGPR[CMIPS::SP].nV0 + 0x14,
                                  m_moduleDataAddr + offsetof(MODULEDATA, rpcBuffer));
...
fs::path CMcServ::GetAbsoluteFilePath(unsigned int port, unsigned int slot, const char *name) const
{
    ...
    if (name[0] == SEPARATOR_CHAR) {
        ...
        return Iop::PathUtils::MakeHostPath(Iop::PathUtils::MakeHostPath(mcPath, m_currentDirectory[port].c_str()),
                                            name);
    }
}
...
void CMcServ::SaveState(Framework::CZipArchiveWriter &archive) const
{
    ...
    for (unsigned int i = 0; i < MAX_PORTS; i++) {
        ...
        cardNode->InsertAttribute(
            Framework::Xml::CreateAttributeBoolValue(STATE_MEMCARDS_CARDNODE_KNOWNATTRIBUTE, m_knownMemoryCards[i]));
...
CMcServ::CPathFinder::CPathFinder() : m_index(0)
{
}
...
void CMcServ::CPathFinder::SearchRecurse(const fs::path &path)
{
    ...
    for (fs::directory_iterator elementIterator(path); elementIterator != endIterator; elementIterator++) {
        ...
        if (std::regex_match(relativePathString, m_filterExp)) {
            ...
            if (fs::is_directory(*elementIterator)) {
                ...
                entry.attributes = MC_FILE_0400 | MC_FILE_ATTR_EXISTS | MC_FILE_ATTR_CLOSED | MC_FILE_ATTR_FILE |
                                   MC_FILE_ATTR_READABLE | MC_FILE_ATTR_WRITEABLE | MC_FILE_ATTR_EXECUTABLE;
            }

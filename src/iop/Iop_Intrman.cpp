...
CIntrman::CIntrman(CIopBios &bios, uint8 *ram) : m_bios(bios), m_ram(ram)
{
}
    ...
    switch (functionId) {
        case 4:
            context.m_State.nGPR[CMIPS::V0].nD0 = static_cast<int32>(
                RegisterIntrHandler(context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0,
                                    context.m_State.nGPR[CMIPS::A2].nV0, context.m_State.nGPR[CMIPS::A3].nV0));
            break;
        case 5:
            context.m_State.nGPR[CMIPS::V0].nD0 =
                static_cast<int32>(ReleaseIntrHandler(context.m_State.nGPR[CMIPS::A0].nV0));
            break;
        case 6:
            context.m_State.nGPR[CMIPS::V0].nD0 =
                static_cast<int32>(EnableIntrLine(context, context.m_State.nGPR[CMIPS::A0].nV0));
            break;
        case 7:
            context.m_State.nGPR[CMIPS::V0].nD0 = static_cast<int32>(
                DisableIntrLine(context, context.m_State.nGPR[CMIPS::A0].nV0, context.m_State.nGPR[CMIPS::A1].nV0));
            break;
        case 8:
            context.m_State.nGPR[CMIPS::V0].nD0 = static_cast<int32>(DisableInterrupts(context));
            break;
        case 9:
            context.m_State.nGPR[CMIPS::V0].nD0 = static_cast<int32>(EnableInterrupts(context));
            break;
        case 17:
            context.m_State.nGPR[CMIPS::V0].nD0 =
                static_cast<int32>(SuspendInterrupts(context, context.m_State.nGPR[CMIPS::A0].nV0));
            break;
        case 18:
            context.m_State.nGPR[CMIPS::V0].nD0 =
                static_cast<int32>(ResumeInterrupts(context, context.m_State.nGPR[CMIPS::A0].nV0));
            break;
        case 23:
            context.m_State.nGPR[CMIPS::V0].nD0 = static_cast<int32>(QueryIntrContext(context));
            break;
        default:
...
uint32 CIntrman::RegisterIntrHandler(uint32 line, uint32 mode, uint32 handler, uint32 arg)
{
    ...
    CLog::GetInstance().Print(
        LOGNAME, FUNCTION_REGISTERINTRHANDLER "(line = %d, mode = %d, handler = 0x%08X, arg = 0x%08X);\r\n", line, mode,
        handler, arg);
#endif
...
#endif
    UNION64_32 mask(context.m_pMemoryMap->GetWord(CIntc::MASK0), context.m_pMemoryMap->GetWord(CIntc::MASK1));
    mask.f |= 1LL << line;

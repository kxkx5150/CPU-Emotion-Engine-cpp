...
class CIopBios : public Iop::CBiosBase {
  ...
  private:
    ...
    static_assert(sizeof(VBLANKHANDLER) == 0x10,
                  "Size of VBLANKHANDLER must be 16 bytes. AssembleVblankHandler relies on this.");
    ...
    static_assert(
        sizeof(SYSTEM_INTRHANDLER) == 0x8,
        "Size of SYSTEM_INTRHANDLER must be 8 bytes. Fixed PS2 structure, and we use it for array magic iteration.");

...
CBasicBlock::CBasicBlock(CMIPS &context, uint32 begin, uint32 end)
    : m_begin(begin), m_end(end), m_context(context)
#ifdef AOT_USE_CACHE
      ,
      m_function(nullptr)
#endif
...
void CBasicBlock::Compile()
{
    ...
    Framework::CMemStream stream;
    {
        ...
        if (jitter == nullptr) {
            ...
            for (unsigned int i = 0; i < 4; i++) {
                jitter->SetVariableAsConstant(offsetof(CMIPS, m_State.nGPR[CMIPS::R0].nV[i]), 0);
            }
        }

        jitter->GetCodeGen()->SetExternalSymbolReferencedHandler([&](auto symbol, auto offset, auto refType) {
            this->HandleExternalFunctionReference(symbol, offset, refType);
        });
    ...
    static const auto blockComparer = [](const AOT_BLOCK &item1, const AOT_BLOCK &item2) {
...
void CBasicBlock::CompileRange(CMipsJitter *jitter)
{
    ...
    for (uint32 address = m_begin; address <= m_end; address += 4) {
        m_context.m_pArch->CompileInstruction(address, jitter, &m_context);
        // Sanity check
...
{
    return (m_begin == MIPS_INVALID_PC) && (m_end == MIPS_INVALID_PC);
}
...
void CBasicBlock::HandleExternalFunctionReference(uintptr_t symbol, uint32 offset,
                                                  Jitter::CCodeGen::SYMBOL_REF_TYPE refType)
{

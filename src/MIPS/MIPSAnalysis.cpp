#include <stdio.h>
#include "MIPS/MIPSAnalysis.h"
#include "MIPS/MIPS.h"

CMIPSAnalysis::CMIPSAnalysis(CMIPS *ctx) : m_ctx(ctx)
{
}
...
void CMIPSAnalysis::InsertSubroutine(uint32 start, uint32 end, uint32 stackAllocStart, uint32 stackAllocEnd,
                                     uint32 stackSize, uint32 returnAddrPos)
{
    ...
    while (candidate != end) {
        ...
        if ((opcode & 0xFFFF0000) == 0x27BD0000) {
            ...
            while (tempAddr != end) {
                ...
                if (((opcode & 0xFFFF0000) == 0xAFBF0000) ||    // SW
...
void CMIPSAnalysis::FindSubroutinesByJumpTargets(uint32 start, uint32 end, uint32 entryPoint)
{
    ...
    for (uint32 address = start; address <= end; address += 4) {
        ...
        if ((opcode & 0xFC000000) == 0x0C000000 || (opcode & 0xFC000000) == 0x08000000) {
...
void CMIPSAnalysis::ExpandSubroutines(uint32 executableStart, uint32 executableEnd)
{
    ...
    const auto &findFreeSubroutineEnd = [this](uint32 begin, uint32 end) -> uint32 {

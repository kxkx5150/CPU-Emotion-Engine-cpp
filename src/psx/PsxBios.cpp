...
CPsxBios::CPsxBios(CMIPS &cpu, uint8 *ram, uint32 ramSize)
    : m_cpu(cpu), m_ram(ram), m_ramSize(ramSize),
        ...
        switch (functionId) {
            ...
            case 0x14:
                CLog::GetInstance().Print(LOG_NAME, "longjmp(buffer = 0x%0.8X, value = %i);\r\n",
                                          m_cpu.m_State.nGPR[SC_PARAM0].nV0, m_cpu.m_State.nGPR[SC_PARAM1].nV0);
                break;
            case 0x19:
        ...
        switch (functionId) {
            ...
            case 0x08:
                CLog::GetInstance().Print(LOG_NAME,
                                          "OpenEvent(class = 0x%X, spec = 0x%X, mode = 0x%X, func = 0x%X);\r\n",
                                          m_cpu.m_State.nGPR[SC_PARAM0].nV0, m_cpu.m_State.nGPR[SC_PARAM1].nV0,
                                          m_cpu.m_State.nGPR[SC_PARAM2].nV0, m_cpu.m_State.nGPR[SC_PARAM3].nV0);
                break;
            case 0x09:
...
void CPsxBios::sc_strcpy()
{
    ...
    strcpy(reinterpret_cast<char *>(m_ram + dst), reinterpret_cast<char *>(m_ram + src));

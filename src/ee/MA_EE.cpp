...
CMA_EE::CMA_EE() : CMA_MIPSIV(MIPS_REGSIZE_64)
{
    ...
    m_codeGen->Else();
    {
        if (m_codeGen->GetCodeGen()->Has128BitsCallOperands()) {
            ...
            for (uint32 i = 0; i < 4; i++) {
                ...
                m_codeGen->Add();
                m_codeGen->Call(reinterpret_cast<void *>(&MemoryUtils_GetWordProxy), 2,
                                Jitter::CJitter::RETURN_VALUE_32);
    ...
    m_codeGen->Else();
    {
        if (m_codeGen->GetCodeGen()->Has128BitsCallOperands()) {
            ...
            for (uint32 i = 0; i < 4; i++) {
                ...
                m_codeGen->PushCst(i * 4);
                m_codeGen->Add();
                m_codeGen->Call(reinterpret_cast<void *>(&MemoryUtils_SetWordProxy), 3,
                                Jitter::CJitter::RETURN_VALUE_NONE);
            }
...
void CMA_EE::PPACW()
{
    ...
    //- Elements 1 and 3 in the destination register are unused in the source registers so, it's safe to write them
    //first
...
void CMA_EE::PMADDH()
{
    static const size_t offsets[8] = {offsetof(CMIPS, m_State.nLO[0]),  offsetof(CMIPS, m_State.nLO[1]),
                                      offsetof(CMIPS, m_State.nHI[0]),  offsetof(CMIPS, m_State.nHI[1]),
                                      offsetof(CMIPS, m_State.nLO1[0]), offsetof(CMIPS, m_State.nLO1[1]),
                                      offsetof(CMIPS, m_State.nHI1[0]), offsetof(CMIPS, m_State.nHI1[1])};
...
void CMA_EE::PMFHL_SH()
{
    ...
    static const auto emitClamp = [](CMipsJitter *codeGen, size_t offsetToClamp) {

...
const CPS2OS::SYSCALL_NAME CPS2OS::g_syscallNames[] = {{0x0004, SYSCALL_NAME_EXIT},
...
CPS2OS::CPS2OS(CMIPS &ee, uint8 *ram, uint8 *bios, uint8 *spr, CGSHandler *&gs, CSIF &sif, CIopBios &iopBios)
    : m_ee(ee), m_gs(gs), m_ram(ram), m_bios(bios), m_spr(spr), m_sif(sif), m_libMc2(ram, *this, iopBios),
      m_iopBios(iopBios),
      ...
      m_intcHandlers(reinterpret_cast<INTCHANDLER *>(m_ram + BIOS_ADDRESS_INTCHANDLER_BASE), BIOS_ID_BASE,
                     MAX_INTCHANDLER),
      m_dmacHandlers(reinterpret_cast<DMACHANDLER *>(m_ram + BIOS_ADDRESS_DMACHANDLER_BASE), BIOS_ID_BASE,
                     MAX_DMACHANDLER),
...
{
    static_assert((BIOS_ADDRESS_SEMAPHORE_BASE + (sizeof(SEMAPHORE) * MAX_SEMAPHORE)) <=
                      BIOS_ADDRESS_CUSTOMSYSCALL_BASE,
                  "Semaphore overflow");

    CAppConfig::GetInstance().RegisterPreferenceInteger(PREF_SYSTEM_LANGUAGE,
                                                        static_cast<uint32>(OSD_LANGUAGE::JAPANESE));
}
...
void CPS2OS::Initialize()
{
    ...
    // The BIOS enables TIMER2 and TIMER3 for alarm purposes and others, some games (ex.: SoulCalibur 3, Ape Escape 2)
    // assume timer is counting.
    m_ee.m_pMemoryMap->SetWord(CTimer::T2_MODE,
                               CTimer::MODE_COUNT_ENABLE | CTimer::MODE_EQUAL_FLAG | CTimer::MODE_OVERFLOW_FLAG);
    m_ee.m_pMemoryMap->SetWord(CTimer::T3_MODE, CTimer::MODE_CLOCK_SELECT_EXTERNAL | CTimer::MODE_COUNT_ENABLE |
                                                    CTimer::MODE_EQUAL_FLAG | CTimer::MODE_OVERFLOW_FLAG);
}
...
{
    return m_ee.CanGenerateInterrupt() && ((m_currentThreadId == m_idleThreadId) || m_idleEvaluator.IsIdle());
}
...
void CPS2OS::LoadExecutableInternal()
{
    ...
    for (uint32 address = minAddr; address < maxAddr; address += 4) {
        ...
        if (opcode == 0x0000000C) {
            ...
            if ((jr == 0x03E00008) && (addiu & 0xFFFF0000) == 0x24030000) {
                // We have it!
...
void CPS2OS::AssembleInterruptHandler()
{
    ...
    // Process INTC interrupt
    {
        ...
        static const auto generateIntHandler = [](CMIPSAssembler &assembler, uint32 line) {
...
void CPS2OS::AlarmUpdateCompare()
{
    ...
    // Enable TIMER3 INTs
    m_ee.m_pMemoryMap->SetWord(CTimer::T3_MODE, CTimer::MODE_CLOCK_SELECT_EXTERNAL | CTimer::MODE_COUNT_ENABLE |
                                                    CTimer::MODE_EQUAL_FLAG | CTimer::MODE_EQUAL_INT_ENABLE);
...
void CPS2OS::HandleInterrupt(int32 cpuIntLine)
{
    ...
    m_ee.m_State.nCOP0[CCOP_SCU::CAUSE] &=
        ~(CCOP_SCU::CAUSE_EXCCODE_MASK | CCOP_SCU::CAUSE_IP_2 | CCOP_SCU::CAUSE_IP_3);
    ...
    for (uint32 i = 0; i < MIPSSTATE::TLB_ENTRY_MAX; i++) {
        ...
        if (vpn == tlbVpn) {
            ...
            if ((entryLo & CCOP_SCU::ENTRYLO_VALID) == 0) {
                ...
                context->m_State.nCOP0[CCOP_SCU::CAUSE] |=
                    isWrite ? CCOP_SCU::CAUSE_EXCCODE_TLBS : CCOP_SCU::CAUSE_EXCCODE_TLBL;
...
void CPS2OS::sc_ReleaseWaitThread()
{
    ...
    if (!thread) {
        CLog::GetInstance().Warn(LOG_NAME,
                                 SYSCALL_NAME_RELEASEWAITTHREAD ": Used on thread that doesn't exist (%d).\r\n", id);
        m_ee.m_State.nGPR[SC_RETURN].nD0 = static_cast<int32>(-1);
        return;
    }
...
void CPS2OS::sc_WakeupThread()
{
    ...
    if ((thread->status == THREAD_SLEEPING) || (thread->status == THREAD_SUSPENDED_SLEEPING)) {
        switch (thread->status) {
            case THREAD_SLEEPING:
                thread->status = THREAD_RUNNING;
                LinkThread(id);
...
void CPS2OS::sc_SuspendThread()
{
    ...
    if ((thread->status == THREAD_ZOMBIE) || (thread->status == THREAD_SUSPENDED) ||
        (thread->status == THREAD_SUSPENDED_WAITING) || (thread->status == THREAD_SUSPENDED_SLEEPING)) {
        m_ee.m_State.nGPR[SC_RETURN].nD0 = static_cast<int32>(-1);
        return;
    }
...
void CPS2OS::sc_ResumeThread()
{
    ...
    if ((thread->status == THREAD_ZOMBIE) || (thread->status == THREAD_RUNNING) || (thread->status == THREAD_WAITING) ||
        (thread->status == THREAD_SLEEPING)) {
        m_ee.m_State.nGPR[SC_RETURN].nD0 = static_cast<int32>(-1);
        return;
    }
...
void CPS2OS::sc_SetupThread()
{
    ...
    uint32 threadId = -1;
    if ((m_currentThreadId == 0) || (m_currentThreadId == m_idleThreadId)) {
...
void CPS2OS::sc_GetOsdConfigParam()
{
    ...
    configParam.screenType =
        static_cast<uint32>(widescreenOutput ? OSD_SCREENTYPE::RATIO_16_9 : OSD_SCREENTYPE::RATIO_4_3);
...
void CPS2OS::sc_SifDmaStat()
{
    ...
    int64 timerDiff =
        static_cast<uint64>(m_ee.m_State.nCOP0[CCOP_SCU::COUNT]) - static_cast<uint64>(m_sifDmaTimes[queueIdx]);
...
void CPS2OS::HandleSyscall()
{
    ...
    if (callInstruction != 0x0000000C) {
        // This will happen if an ADDIU R0, R0, $x instruction is encountered. Not sure if there's a use for that on the
        // EE
        CLog::GetInstance().Warn(
            LOG_NAME,
            "System call exception occured but no SYSCALL instruction found (addr = 0x%08X, opcode = 0x%08X).\r\n",
...
std::string CPS2OS::GetSysCallDescription(uint8 function)
{
    ...
    switch (function) {
        case 0x02:
            sprintf(description, "GsSetCrt(interlace = %i, mode = %i, field = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0],
                    m_ee.m_State.nGPR[SC_PARAM1].nV[0], m_ee.m_State.nGPR[SC_PARAM2].nV[0]);
            break;
        case 0x04:
        ...
        case 0x06:
            sprintf(description, SYSCALL_NAME_LOADEXECPS2 "(exec = 0x%08X, argc = %d, argv = 0x%08X);",
                    m_ee.m_State.nGPR[SC_PARAM0].nV[0], m_ee.m_State.nGPR[SC_PARAM1].nV[0],
                    m_ee.m_State.nGPR[SC_PARAM2].nV[0]);
            break;
        case 0x07:
            sprintf(description, SYSCALL_NAME_EXECPS2 "(pc = 0x%08X, gp = 0x%08X, argc = %d, argv = 0x%08X);",
                    m_ee.m_State.nGPR[SC_PARAM0].nV[0], m_ee.m_State.nGPR[SC_PARAM1].nV[0],
                    m_ee.m_State.nGPR[SC_PARAM2].nV[0], m_ee.m_State.nGPR[SC_PARAM3].nV[0]);
            break;
        case 0x0D:
        ...
        case 0x10:
            sprintf(description,
                    SYSCALL_NAME_ADDINTCHANDLER "(cause = %i, address = 0x%08X, next = 0x%08X, arg = 0x%08X);",
                    m_ee.m_State.nGPR[SC_PARAM0].nV[0], m_ee.m_State.nGPR[SC_PARAM1].nV[0],
                    m_ee.m_State.nGPR[SC_PARAM2].nV[0], m_ee.m_State.nGPR[SC_PARAM3].nV[0]);
            break;
        case 0x11:
        ...
        case 0x13:
            ...
            break;
        case 0x14:
            sprintf(description, SYSCALL_NAME_ENABLEINTC "(cause = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x15:
            sprintf(description, SYSCALL_NAME_DISABLEINTC "(cause = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x16:
            sprintf(description, SYSCALL_NAME_ENABLEDMAC "(channel = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x17:
            sprintf(description, SYSCALL_NAME_DISABLEDMAC "(channel = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x18:
            ...
            break;
        case 0x1A:
            sprintf(description, SYSCALL_NAME_IENABLEINTC "(cause = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x1B:
            sprintf(description, SYSCALL_NAME_IDISABLEINTC "(cause = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x1C:
            sprintf(description, SYSCALL_NAME_IENABLEDMAC "(channel = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x1D:
            sprintf(description, SYSCALL_NAME_IDISABLEDMAC "(channel = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x1E:
            ...
            break;
        case 0x1F:
            sprintf(description, SYSCALL_NAME_IRELEASEALARM "(id = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x20:
            sprintf(description, SYSCALL_NAME_CREATETHREAD "(thread = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x21:
            sprintf(description, SYSCALL_NAME_DELETETHREAD "(id = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x22:
        ...
        case 0x24:
            ...
            break;
        case 0x25:
            sprintf(description, SYSCALL_NAME_TERMINATETHREAD "(id = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x29:
        ...
        case 0x2B:
            ...
            break;
        case 0x2D:
            sprintf(description, SYSCALL_NAME_RELEASEWAITTHREAD "(id = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x2E:
            sprintf(description, SYSCALL_NAME_IRELEASEWAITTHREAD "(id = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x2F:
        ...
        case 0x32:
            ...
            break;
        case 0x33:
            sprintf(description, SYSCALL_NAME_WAKEUPTHREAD "(id = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x34:
            sprintf(description, SYSCALL_NAME_IWAKEUPTHREAD "(id = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x35:
            sprintf(description, SYSCALL_NAME_CANCELWAKEUPTHREAD "(id = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x36:
            sprintf(description, SYSCALL_NAME_ICANCELWAKEUPTHREAD "(id = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x37:
            sprintf(description, SYSCALL_NAME_SUSPENDTHREAD "(id = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x38:
            sprintf(description, SYSCALL_NAME_ISUSPENDTHREAD "(id = %d);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x39:
            sprintf(description, SYSCALL_NAME_RESUMETHREAD "(id = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x3C:
            sprintf(description,
                    "SetupThread(gp = 0x%08X, stack = 0x%08X, stack_size = 0x%08X, args = 0x%08X, root_func = 0x%08X);",
                    m_ee.m_State.nGPR[SC_PARAM0].nV[0], m_ee.m_State.nGPR[SC_PARAM1].nV[0],
                    m_ee.m_State.nGPR[SC_PARAM2].nV[0], m_ee.m_State.nGPR[SC_PARAM3].nV[0],
        ...
        case 0x3E:
            ...
            break;
        case 0x40:
            sprintf(description, SYSCALL_NAME_CREATESEMA "(sema = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x41:
            sprintf(description, SYSCALL_NAME_DELETESEMA "(semaid = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x42:
            sprintf(description, SYSCALL_NAME_SIGNALSEMA "(semaid = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x43:
            sprintf(description, SYSCALL_NAME_ISIGNALSEMA "(semaid = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x44:
            sprintf(description, SYSCALL_NAME_WAITSEMA "(semaid = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x45:
            sprintf(description, SYSCALL_NAME_POLLSEMA "(semaid = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x46:
            sprintf(description, SYSCALL_NAME_IPOLLSEMA "(semaid = %i);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x47:
...
//		sprintf(description, SYSCALL_NAME_FLUSHCACHE "();");
        ...
        case 0x70:
            ...
            break;
        case 0x71:
            sprintf(description, SYSCALL_NAME_GSPUTIMR "(GS_IMR = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x73:
        ...
        case 0x78:
            ...
            break;
        case 0x79:
            sprintf(description, "SifSetReg(register = 0x%08X, value = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0],
                    m_ee.m_State.nGPR[SC_PARAM1].nV[0]);
            break;
        case 0x7A:
            sprintf(description, "SifGetReg(register = 0x%08X);", m_ee.m_State.nGPR[SC_PARAM0].nV[0]);
            break;
        case 0x7C:
    ...
    static BiosDebugObjectInfoMap objectDebugInfo = [] {
        BiosDebugObjectInfoMap result;
        {
            ...
            info.fields = {
                ...
                {"Option", BIOS_DEBUG_OBJECT_FIELD_TYPE::UINT32,
                 BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::POSSIBLE_STR_POINTER},
        ...
        }
        {
            ...
            info.fields          = {
                         ...
                         {"Address", BIOS_DEBUG_OBJECT_FIELD_TYPE::UINT32,
                          BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::LOCATION | BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::TEXT_ADDRESS},
        ...
        }
        {
            ...
            info.fields          = {
                         ...
                         {"Location", BIOS_DEBUG_OBJECT_FIELD_TYPE::UINT32,
                          BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::LOCATION | BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::TEXT_ADDRESS},
                         {"RA", BIOS_DEBUG_OBJECT_FIELD_TYPE::UINT32,
                          BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::RETURN_ADDRESS | BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::HIDDEN},
                         {"SP", BIOS_DEBUG_OBJECT_FIELD_TYPE::UINT32,
                          BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::STACK_POINTER | BIOS_DEBUG_OBJECT_FIELD_ATTRIBUTE::HIDDEN},

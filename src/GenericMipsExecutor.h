...
template <typename BlockLookupType, uint32 instructionSize = 4> class CGenericMipsExecutor : public CMipsExecutor {
  public:
    enum
    {
    ...
    CGenericMipsExecutor(CMIPS &context, uint32 maxAddress)
        : m_emptyBlock(std::make_shared<CBasicBlock>(context, MIPS_INVALID_PC, MIPS_INVALID_PC)), m_context(context),
          m_maxAddress(maxAddress), m_addressMask(maxAddress - 1), m_blockLookup(m_emptyBlock.get(), maxAddress)
    {
        m_emptyBlock->Compile();
        ResetBlockOutLinks(m_emptyBlock.get());

        assert(!context.m_emptyBlockHandler);
        context.m_emptyBlockHandler = [&](CMIPS *context) {
  ...
  protected:
    ...
    void SetupBlockLinks(uint32 startAddress, uint32 endAddress, uint32 branchAddress)
    {
...

        {
            ...
            auto       link =
                m_blockOutLinks.insert(std::make_pair(nextBlockAddress, BLOCK_OUT_LINK{linkSlot, startAddress, false}));
        ...
        if (branchAddress != 0) {
            ...
            auto       link =
                m_blockOutLinks.insert(std::make_pair(branchAddress, BLOCK_OUT_LINK{linkSlot, startAddress, false}));
            block->SetOutLink(linkSlot, link);
    ...
    virtual void PartitionFunction(uint32 startAddress)
    {
        ...
        for (uint32 address = startAddress; address < endAddress; address += 4) {
            ...
            if (branchType == MIPS_BRANCH_NORMAL) {
                ...
                // If it is, don't include it in this block. This will make the behavior coherent between linked and
                // non-linked blocks. Branch instructions don't modify registers, it should be ok not to execute them if
                // they are in a delay slot
                {
    ...
    {
        auto orphanBlockLinkSlot = [&](LINK_SLOT linkSlot) {
    ...
    void ClearActiveBlocksInRangeInternal(uint32 start, uint32 end, CBasicBlock *protectedBlock)
    {
        ...
        for (uint32 address = scanStart; address < scanEnd; address += instructionSize) {
            ...
            if (block == protectedBlock)
                continue;
            if (!RangesOverlap(block->GetBeginAddress(), block->GetEndAddress(), start, end))
                continue;
        ...
        if (!clearedBlocks.empty()) {
            m_blocks.remove_if(
                [&](const BasicBlockPtr &block) { return clearedBlocks.find(block.get()) != std::end(clearedBlocks); });
        }
    }
